...
func pinnedMenuContext(taskID string) gtk.Menu {
        menu := gtk.NewMenu()
        menuItem := gtk.NewMenuItemWithLabel(T("Unpin"))
...
func launcherButton(position *string) *gtk.Box {
        ...
        if !*noLauncher && *launcherCmd != "" {
                ...
                // Поддержка эмодзи-иконки: если -ico начинается с "emoji:", используем текст-эмодзи
                emojiStr := ""
                if strings.HasPrefix(*ico, "emoji:") {
                        emojiStr = strings.TrimPrefix(*ico, "emoji:")
                }

                if emojiStr != "" {
                        // Используем кнопку с текстом-эмодзи вместо иконки
                        button.SetLabel(emojiStr)
                        button.SetObjectProperty("name", "launcher-emoji")
                        // Убираем изображение, показываем только текст
                        button.SetAlwaysShowImage(false)
                } else {
                        var pixbuf *gdkpixbuf.Pixbuf
                        var e error
                        if *ico == "" {
                        ...
                        if e == nil {
                                ...
                                // Если иконка не найдена, используем текстовую кнопку
                                button.SetLabel("☰")
                                button.SetObjectProperty("name", "launcher-emoji")
                                button.SetAlwaysShowImage(false)
                        }
                ...
                button.Connect("clicked", func() {
                        ...
                        go func() {
                                err := cmd.Run()
                                if err != nil {
                                        log.Warnf(T("Unable to start program")+": %s", err.Error())
                                }
                        }()
...
func clientMenuContext(class string, instances []client) gtk.Menu {
        ...
        for _, instance := range instances {
                ...
                subitem := gtk.NewMenuItemWithLabel(T("closewindow"))
                submenu.Append(subitem)
                subitem.Connect("activate", func() {
                        closeWindow(a)
                })

                subitem = gtk.NewMenuItemWithLabel(T("togglefloating"))
                submenu.Append(subitem)
                subitem.Connect("activate", func() {
                ...
                subitem = gtk.NewMenuItemWithLabel(T("fullscreen"))
                submenu.Append(subitem)
                subitem.Connect("activate", func() {
        ...
        item := gtk.NewMenuItemWithLabel(T("New window"))
        ...
        closeAllWindows.SetLabel(T("Close all windows"))
...
func launch(ID string) {
        ...
        if err := cmd.Start(); err != nil {
                log.Error(T("Unable to launch command!"), err.Error())
        }
