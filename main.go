...
// Flags — описания будут переведены после определения языка в main()
...
var hotspotDelay *int64
var hotspotLayer *string
var ico *string
var ignoreWorkspaces *string
var imgSize *int
var launcherCmd *string
var launcherPos *string
var layer *string
...
// initFlags регистрирует флаги с локализованными описаниями
func initFlags() {
        alignment = flag.String("a", "center", T("Alignment in full width/height: \"start\", \"center\" or \"end\""))
        autohide = flag.Bool("d", false, T("auto-hiDe: show dock when hotspot hovered, close when left or a button clicked"))
        cssFileName = flag.String("s", "style.css", T("Styling: css file name"))
        debugFlag = flag.Bool("debug", false, T("turn on debug messages"))
        displayVersion = flag.Bool("v", false, T("display Version information"))
        exclusive = flag.Bool("x", false, T("set eXclusive zone: move other windows aside; overrides the \"-l\" argument"))
        full = flag.Bool("f", false, T("take Full screen width/height"))
        ignoreClasses = flag.String("g", "", T("quote-delimited, space-separated class list to iGnore in the dock"))
        hotspotDelay = flag.Int64("hd", 20, T("Hotspot Delay [ms]; the smaller, the faster mouse pointer needs to enter hotspot for the dock to appear; set 0 to disable"))
        hotspotLayer = flag.String("hl", "overlay", T("Hotspot Layer \"overlay\" or \"top\""))
        ico = flag.String("ico", "", T("alternative name or path for the launcher ICOn"))
        ignoreWorkspaces = flag.String("iw", "", T("Ignore the running applications on these Workspaces based on the workspace's name or id, e.g. \"special,10\""))
        imgSize = flag.Int("i", 48, T("Icon size"))
        launcherCmd = flag.String("c", "nwg-drawer", T("Command assigned to the launcher button"))
        launcherPos = flag.String("lp", "end", T("Launcher button position, 'start' or 'end'"))
        layer = flag.String("l", "overlay", T("Layer \"overlay\", \"top\" or \"bottom\""))
        marginBottom = flag.Int("mb", 0, T("Margin Bottom"))
        marginLeft = flag.Int("ml", 0, T("Margin Left"))
        marginRight = flag.Int("mr", 0, T("Margin Right"))
        marginTop = flag.Int("mt", 0, T("Margin Top"))
        noLauncher = flag.Bool("nolauncher", false, T("don't show the launcher button"))
        numWS = flag.Int64("w", 10, T("number of Workspaces you use"))
        position = flag.String("p", "bottom", T("Position: \"bottom\", \"top\" \"left\" or \"right\""))
        resident = flag.Bool("r", false, T("Leave the program resident, but w/o hotspot"))
        targetOutput = flag.String("o", "", T("name of Output to display the dock on"))
        allowMultipleInstances = flag.Bool("m", false, T("allow Multiple instances of the dock (skip lock file check)"))
}
...
func main() {
        // Определяем язык до парсинга флагов, чтобы описания были локализованы
        Lang = detectLang()
        initFlags()
        ...
        flag.Usage = func() {
                ...
                fmt.Fprintf(flag.CommandLine.Output(), " SIGRTMIN+1 (%s): %s\n", sigToggle, T("toggle dock visibility (USR1 has been deprecated)"))
                fmt.Fprintf(flag.CommandLine.Output(), " SIGRTMIN+2 (%s): %s\n", sigShow, T("show the dock"))
                fmt.Fprintf(flag.CommandLine.Output(), " SIGRTMIN+3 (%s): %s\n", sigHide, T("hide the dock"))
        }
        ...
        if *autohide && *resident {
                log.Warn(T("autohiDe and Resident arguments are mutually exclusive, ignoring -d!"))
                *autohide = false
        }

        if *displayVersion {
                fmt.Printf("%s %s\n", T("nwg-dock-hyprland version"), version)
                os.Exit(0)
        }

        his = os.Getenv("HYPRLAND_INSTANCE_SIGNATURE")
        if his == "" {
                log.Fatal(T("HYPRLAND_INSTANCE_SIGNATURE not found, terminating."))
                os.Exit(1)
        }
        ...
        if *autohide {
                log.Info(T("Starting in autohiDe mode"))
        }
        if *resident {
                log.Info(T("Starting in resident mode"))
        }
        if *ignoreClasses != "" {
        ...
        go func() {
                for {
                        ...
                        case syscall.SIGTERM:
                                log.Info(T("SIGTERM received, bye bye!"))
                                gtk.MainQuit()
                        case syscall.SIGUSR1:
                                log.Warn(T("SIGUSR1 for toggling visibility is deprecated, use SIGRTMIN+1"))
        ...
        if !*allowMultipleInstances {
                ...
                if e != nil {
                        ...
                        if err == nil {
                                ...
                                if err == nil {
                                        if *autohide || *resident {
                                                log.Info(T("Running instance found, terminating..."))
                                        } else {
                                                _ = syscall.Kill(i, sigToggle)
                                                log.Info(T("Sending sigToggle to running instance and bye, bye!"))
                                        }
                                }
                        } else {
        ...
        if !*noLauncher && *launcherCmd == "" {
                ...
                if *launcherCmd != "" {
                        ...
                        log.Info(T("Neither 'nwg-drawer' nor 'nwggrid' command found, and no other launcher specified; hiding the launcher button."))
                }
        }

        dataHome, err = getDataHome()
        if err != nil {
                log.Fatal(T("Error getting data directory:"), err)
        }
        ...
        if cacheDirectory == "" {
                log.Panic(T("Couldn't determine cache directory location"))
        }
        ...
        go func() {
                ...
                if err != nil {
                        fmt.Println(T("Error connecting to the socket:"), err)
                        os.Exit(1)
                }
                defer conn.Close()
                ...
                for {
                        ...
                        n, err := conn.Read(buf)
                        if err != nil {
                                fmt.Println(T("Error reading from socket2:"), err)
                        }
